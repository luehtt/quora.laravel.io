    ...
    <div id="report-modal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="main">
            <div class="modal-content">
                ...
                {!! Form::open(['route' => ['user.reply.report', $item->id], 'method' => 'POST']) !!}
        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-sm-10 col-12 my-auto">
                    <p class="mb-2">{{ $item->user->name }} phản hồi lúc {{ $item->created_at }}</p>
                    <p><a href="{{ url('/discussion/'.$discussion->slug) }}">{{ $discussion->title }}</a></p>
                </div>
            </div>
        </div>
    ...
    <div class="mb-4">
        ...
        @else
            <button type="button" class="btn btn-lg btn-danger ml-0" data-toggle="modal" data-target="#report-modal">
                Gửi tố cáo<i class="fas fa-exclamation-triangle ml-2"></i></button>
        @endif
        <a class="btn btn-lg btn-mdb-color mr-0 text-white float-right" href="{{ url('/discussion/'.$discussion->slug) }}">
